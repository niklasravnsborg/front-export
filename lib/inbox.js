...
module.exports = function exportInbox(inboxId, basePath) {
	...
	return getConversationsForInbox(inboxId)
		...
		.then(conversations =>
			...
			mkdirp.mkdirpAsync(basePath).then(() =>
				// Write the `conversations.json` file to it
				fs.writeFileAsync(path.join(basePath, 'conversations.json'), JSON.stringify(conversations, null, 2)).then(() => conversations)
			)
		)
		.then(conversations => Promise.all(
			conversations
				.map(c => c.messages)
				.map(messages => Promise.all(messages.forEach(m => downloadAttachmentsForMessage(m, basePath))))
		));
};
